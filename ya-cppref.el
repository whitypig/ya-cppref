...
(defconst ya-cppref-path-to-en "en.cppreference.com/w/")
(defconst ya-cppref-path-to-en-cpp (concat ya-cppref-path-to-en "cpp/"))

;;;;;;;;;;;;;;;;;;;; Functions ;;;;;;;;;;;;;;;;;;;;
(defun ya-cppref-init ()
  "Return a hash table with its contents being `(node . (path1
path2))'."
  (let ((dir (concat ya-cppref-path-to-doc-root ya-cppref-path-to-en)))
  ...
  (flet ((f (l)
            (mapcan (lambda (e)
                      (if (file-directory-p e)
                        ...
                        (list e)))
                    l)))
    (sort (f (directory-files (expand-file-name (concat docroot ya-cppref-path-to-en-cpp))
                        t
                        "[^.]$"))
          #'string<)))
